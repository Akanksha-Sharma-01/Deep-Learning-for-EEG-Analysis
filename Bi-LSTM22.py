...
def encoder(input):
	x = tf.keras.layers.Conv2D(kernel_size=(1, 35),   filters=25, strides = (1,2), activation = 'relu')(input)
	...
	x = tf.keras.layers.Bidirectional(layers.LSTM(22, return_sequences=True),input_shape=(30, 25))(x)
	x = tf.keras.layers.Bidirectional(layers.LSTM(22))(x)
	#x = transformer(x, embed_dim2, 0.5, ff_dim)
	x = tf.keras.layers.Flatten()(x)
	return x

def classifier(input):
	x = layers.Dense(70, activation="sigmoid")(input)
...
model.summary()
history = model.fit(TrainData, TrainLabel, batch_size=40, epochs=1000, validation_split=0, validation_data=(TestData, TestLabel),verbose = 2, callbacks=my_callbacks)
model.save("/home/arnav/Desktop/Akanksha/40ClassModel1Bi_LSTM22.h5")
...
plt.subplot(3,1,3)
plt.plot(TrAcc,ValAcc,label="Accuracy")
plt.plot(TrLoss,ValLoss,label="Loss")
plt.legend()
plt.grid()
plt.xlabel("Training")
