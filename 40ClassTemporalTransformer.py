...
def positionalEmbedding(input,maxlen,embed_dim):
	...
	emm = layers.Embedding(input_dim=maxlen, output_dim=embed_dim, input_length=maxlen)(position)
...
def encoder(input):
	...
	x = tf.keras.layers.Permute((2,1),input_shape=(30,25))(x)
	x = positionalEmbedding(x,750,1)
	x = tf.keras.layers.Reshape((25,30))(x)
...
model.summary()
history = model.fit(TrainData, TrainLabel, batch_size=20, epochs=17, validation_split=0, validation_data=(TestData, TestLabel),verbose = 2)#, callbacks=my_callbacks)
model.save("/home/arnav/Desktop/Akanksha/40ClassTemporalTransformer17epoch.h5")
